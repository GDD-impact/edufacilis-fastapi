...
async def get_periodic_tasks(session: AsyncSession = Depends(async_get_db)):
    """
...
def create_cron_schedule(scheduling_type, start_datetime, end_datetime):
    ...
    if scheduling_type == "daily":
        ...
        cron_value = crontab(day_of_week=start_datetime.weekday(
        ), hour=start_time.hour, minute=start_time.minute)
    elif scheduling_type == "weekdays":
        cron_value = crontab(day_of_week="mon-fri",
                             hour=start_time.hour, minute=start_time.minute)
    elif scheduling_type == "monthly":
        cron_value = crontab(day_of_month=start_datetime.day,
                             hour=start_time.hour, minute=start_time.minute)
    elif scheduling_type == "yearly":
        cron_value = crontab(day_of_year=start_datetime.timetuple(
        ).tm_yday, hour=start_time.hour, minute=start_time.minute)
